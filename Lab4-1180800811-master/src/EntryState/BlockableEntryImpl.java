...
public class BlockableEntryImpl implements BlockableEntry {
	...
	public void checkRep() {
		...
		if(timeslots.size() > 1 ) {
			for(int i = 0 ; i < timeslots.size() - 1 ; i ++	) {
				assert timeslots.get(i).getdate2().compareTo(timeslots.get(i+1).getdate1()) < 0 ;//时间对递增
			}
		}
	}
	...
	public void setTimeslots(List<Timeslot> time) {
		...
		time1.addAll(time) ;
		Collections.sort(time1, new Compare1());
	...
	public boolean equals(Object obj) {
		...
		return timeslots.equals(other.timeslots);
	}
