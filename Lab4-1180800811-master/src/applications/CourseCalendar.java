...
public class CourseCalendar {
	/*
	 * RI: 
	 * resource , location , courseEntry,time is not null
	 * 
	 ...
	 * @param time2 课程结束时间
	 * @param loc 课程的教室位置
	 * @throws cancelPlanningEntryException 取消某计划项的时候，如果该计划项的当前状态不允许取消
	 */
	public String cancelPlanningEntry(String Name ,Calendar time1 , Calendar time2  , Location loc) throws cancelPlanningEntryException  {
	 ...
	 * @throws feipeiResourceException 分配资源后会导致与已有的其他计划项产生“资源独占冲突”
	 */
	public int FeiPeiResource(String Name , String idNumber , String name ,boolean sex , String title	) throws feipeiResourceException {
		int x = 0 ;
		...
		if(!APIs.PlanningEntryAPIs.checkResourceExclusiveConflict(s, new checkResourceExclusiveConflict1())) {//计划项存在冲突
			throw new feipeiResourceException(teacher) ;
		}
		 return 0 ;//分配成功
	}
	/**
	 ...
	 * @throws changeLocationException  为某计划项变更位置的时候，如果变更后会导致与已有的其他计划项产生“位置独占冲突”
	 */
	public boolean changeLocationEntry(String Name , Location loc1 , Location loc2) throws changeLocationException {
		for(CourseEntry e : courseEntry) {
			if(e.getName().equals(Name)) {
				if(e.getLocation().equals(loc1)) {
					e.setLocation(loc2);
					List<PlanningEntry> t = new ArrayList<PlanningEntry>();
					t.addAll(courseEntry) ;
					if(!APIs.PlanningEntryAPIs.checkLocationConflict(t)) {
						throw new changeLocationException(loc1, loc2);
					}
	 ...
	 * @throws deleteLocationException 删除某位置的时候，如果有尚未结束的计划项在该位置执行
	 */
	 ...
	 * @param title 教师职称
	 * @return true: 删除成功 ， false :删除失败
	 * @throws deleteResourceException 删除某资源的时候，有尚未结束的计划项正在占用该资源
	 */
	public boolean deleteResource(String name , String Number , boolean s , String title) throws deleteResourceException  {
		Teacher tea = new Teacher(Number, name, s, title);
		if(!resource.contains(tea)) {
			return false ;
		}
		for(CourseEntry e : courseEntry) {
			if(e.getResource().equals(tea)) {
				if(!e.getState().equals(ENDED.instance) || e.getState().equals(CANCELLED.instance)) {
					throw new deleteResourceException(tea) ;
				}
			}
		}
		resource.remove(tea);
