...
public class TrainEntryTest {
	...
	public void setResourceTest() {
		TrainEntry train = new TrainEntry("复兴号") ;
		Railway r1 = new Railway("aa", Type.BUSINESS, 20, 2019);
		Railway r2 = new Railway("bb", Type.BUSINESS, 30, 2019);
		List<Railway> msre1 = new ArrayList<Railway>() ;
		msre1.add(r1);
		msre1.add(r2);
	...
	/*
	 * 测试addLocation方法,addTimeslots方法
	 */
	@Test
	public void addLocationTest() {
		TrainEntry train = new TrainEntry();
		train.setName("s");
		train.addLocation(new Location("ss"));
		assertTrue(train.getLocation().contains(new Location("ss")));
		Calendar time1 =Calendar.getInstance() ;
		time1.set(2020, 5-1, 6, 11, 22);
		Calendar time2 =Calendar.getInstance() ;
		time2.set(2020, 5-1, 6, 12, 22);
		Timeslot time = new Timeslot(time1, time2);
		train.addTimeslots(time);
		assertTrue(train.getTimeslots().contains(time));
	}
