...
public class TrainScheduleBoard implements Iterable<PlanningEntry>{
	...
	public void visualize(int x) {
		...
		Map<Calendar,TrainEntry> train = new TreeMap<Calendar, TrainEntry>() ;
		if(x == 0) {
			train = gettrain1() ;
		}else {
			train = gettrain2();
		...
		Iterator<Calendar> TRAIN = train.keySet().iterator();//得到所有和当前时间相差1个小时的计划项及其对应的时间关系图
		while(TRAIN.hasNext()) {
			Calendar TIME= TRAIN.next();
			if(Math.abs(TIME.getTime().getTime() - this.time.getTime().getTime()) <= 3600000) {
				train1.put(TIME, train.get(TIME)) ;
			}
		}
		String[] columNames = { "序号" , "时间" , "高铁车次" ,"行程" , "状态"} ;//表格第一行信息
